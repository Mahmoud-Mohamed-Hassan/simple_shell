#include "shell.h"
/**
 * my_builtin_check - check if cmd is builtin cmd
 *
 * @cmd: int parameter
 *
...
int my_builtin_check(char *cmd)
{
	...
	for (index = 0; builtin_commands[index]; index++)
		if (str_compare2(cmd, builtin_commands[index]) == 0)
			return (1);
	return (0);
}
/**
...
{
	if (str_compare2(cmd[0], "exit") == 0)
	{
		exit_my_shell(cmd, argv, status, index);
	}
	else if (str_compare2(cmd[0], "env") == 0)
	{
...
void exit_my_shell(char **cmd, char **argv, int *status, int index)
{
	...
	{
		if (is_postive(cmd[1]))
			my_exit_value = my_atoi(cmd[1]);

		else
		{
			index_s = my_itoa(index);
			my_writeErr(argv, index_s, my_msg, cmd);

			free(index_s);
			my_FreeArrOfString(cmd);
			(*status) = 2;
			return;
		}
	}
